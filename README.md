...
An alternative execution loop for a feature that decomposes into independently-shippable vertical slices, where the commits themselves are the plan. No Jira stack, no squash, no manifest file: each slice is one commit carrying `Slice-Id` / `Depends-On` trailers, and `git log` is the ledger. Shared formats and the reasoning behind them live in `commands/_sliced-format.md`; every derivation over the ledger lives in four CLIs (`read-ledger`, `slice-scope`, `slice-replay`, `slice-review`) rather than in prose the agent is asked to reproduce.

#### `/build-sliced [spec ref] [BASE]`

Builds a whole feature as dependency-ordered vertical slices on one branch — each a thin end-to-end increment (its own contract, a consumer, its test) that passes CI on its own, ordered so a slice reusing an earlier slice's code comes after it. `<spec ref>` is a Jira key, `docs/tdds/{slug}.md`, or `.plans/ears-{slug}.md` — never free text. Refuses work that doesn't decompose into 2+ independently-shippable increments and names the better home (`/refactor`, `/ticket-work`, or "too small to slice").
...
- **The bar is one fixed thing**: the slice passes the project's full local CI gate — build/compile, lint, typecheck, the whole suite, any coverage gate — against the tree at its commit. The bar *is* CI run locally, which is what makes a green slice independently mergeable. A clean cherry-pick doesn't inherit yesterday's green bar, which is why only `regenerated-identical` may skip it.
...
- **Each slice gets its own PR**, stacked on the previous slice's branch (`<BRANCH>--<id>`) by **commit** order, not by `Depends-On` — a linear stack has one predecessor per commit, and it already contains everything the slice built on. So a slice can merge, and pass CI, as soon as it's green without the rest landing first. No umbrella PR for the whole branch — the per-slice PRs are the only PRs, force-pushed and restacked (in `replaySpan` scope) after each replay. When the stack is settled it hands off: merge bottom-up starting at the first slice, repointing each successor's base to `<BASE>` before merging it — a human step the command never drives.
