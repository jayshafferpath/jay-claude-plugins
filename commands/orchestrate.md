...
> **Decision table reference**: The canonical rule list lives in `cli/lib/classify-actions.js` (`classifyTicket`). Briefly: rule 0 = merged **to main** with no branch on record → `cleaned` (cleanup already ran; dispatching `/cleanup` again would only hit its "No branch on record" refusal); rule 0a = merged into a **feature branch** only, no branch on record, `merged/{KEY}` tag present → `promote-to-main` (the tag is all the tag-based replay needs); rule 0b = same shape but no tag either → `stranded` (nothing left to replay onto main — needs a human); rule 1 = `mergedIntoMain` → `cleanup-terminal`; rule 1a = Story-container merged into parent Epic feature branch, phase-1 not yet run → `cleanup-phase-1`; rule 1b = same shape but `phaseOneDone` → `promote-to-main` (phase-1 cleanup already ran, the main PR hasn't landed); rule 1c = **leaf** ticket merged into its container's feature branch → `cleanup-phase-1` when phase-1 hasn't run, `promote-to-main` once `phaseOneDone` (never terminal: a leaf is promoted to main individually, so its branch and Jira state must survive); rule 2 = `ClaudeStackReady` → `awaiting-review`; rule 3 = `ClaudeFailed` → `failed`; rule 4 = `ClaudeExecuting` / `ClaudePlanning` → `in-flight`; rule 5 = `ClaudeReady && eligible` → `ticket-work`; rule 6 = `ClaudeReady && !eligible` → `blocked-on-stack`; rule 7 = idle. Container-blocked overrides every rule.
>
> **Rule 0 is scoped to main, and rule 1c advances on `phaseOneDone`.** Both were sources of the same failure: merged work with no rule that would ever ship it. Rule 0 once fired on `mergedIntoMain || mergedIntoFeature`, so a ticket whose branch phase-1 had deleted read as `cleaned` / not-auto-safe even though it still owed a main promotion. Rule 1c meanwhile re-emitted `cleanup-phase-1` on every pass — the branch survives phase-1, so nothing in the rule ever changed — and the leaf never advanced to promotion. Together they left three commits (NEV-1442, NEV-1616, NEV-1446) merged onto Epic branch NEV-1352 with no open PR to main and no path to one. The `merged/{KEY}` tag is the discriminator in both cases: it survives branch deletion and is exactly what `/promote-to-main` Step 2a replays from.

> **Why rules 1a/1b and 1c are separate**: 1a/1b gate on the ticket's branch *being* the container's feature branch, which describes a stack-container — a Story whose own feature branch was PR'd into a parent Epic's. A leaf (a Story with no children, or a subtask) whose ticket branch merged into its container's feature branch has `branch !== container.featureBranch` and a null `parentFeatureBranch`, so neither fires, and `mergedIntoMain` is false so rule 1 doesn't either. Rule 1c catches that shape — the common one for every child of an Epic — off `mergedIntoFeature` rather than a branch-name comparison. Both paths converge on `cleanup-phase-1`, because a feature-branch merge is not a ship to main in either shape.
>
> **Rule 1c emits `cleanup-phase-1`, not `cleanup-terminal`.** It previously emitted terminal on the premise that a leaf is never promoted separately. That premise is false — `/promote-to-main` Step 1c accepts a leaf ticket key directly, and its container-key path promotes leaves one at a time off `GIT_MERGE_ORDER` — so terminal cleanup marked Stories Done while their code was still only on the Epic branch, and deleted branches the Step 2a rebase fallback still needed. It also could not self-correct: once the branch was deleted, rule 0 classified the ticket `cleaned` (not auto-safe), so nothing revisited it to mark it Done after the work actually landed on main. Rule 1 now handles that, firing on the same ticket once `mergedIntoMain` flips — and rule 0 no longer swallows the feature-branch-only case at all (see rules 0a/0b).
...
- `✓ auto: cleanup-terminal` — safe, will run (PR merged to main; full teardown — branch deleted, Jira → Done).
- `· cleaned` — merged with no branch left on record; cleanup already ran. Nothing to do.
- `✓ auto: cleanup-phase-1` — safe, will run (PR merged into a feature branch rather than main — either a Story-container into its parent Epic's branch or a leaf into its container's; branch and Jira state retained for /promote-to-main, sibling rebase + feature-branch refresh only).
