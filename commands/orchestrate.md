...
>
> **Rule 1c emits `cleanup-phase-1`, not `cleanup-terminal`.** It previously emitted terminal on the premise that a leaf is never promoted separately. That premise is false — `/promote-to-main` Step 1c accepts a leaf ticket key directly, and its container-key path promotes leaves one at a time off `GIT_MERGE_ORDER` — so terminal cleanup marked Stories Done while their code was still only on the Epic branch, and deleted branches the Step 2a rebase fallback still needed. It also could not self-correct: once the branch was deleted, rule 0 classified the ticket `cleaned` (not auto-safe), so nothing revisited it to mark it Done after the work actually landed on main. Rule 1 now handles that, firing on the same ticket once `mergedIntoMain` flips.
